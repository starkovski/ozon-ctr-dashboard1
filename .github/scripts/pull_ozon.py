...
<style>body{font-family:sans-serif;padding:24px;max-width:1100px;margin:0 auto}
h1{margin:0 0 12px} .row{display:flex;gap:12px;align-items:center;margin:16px 0}
select,button{padding:8px 10px} canvas{max-width:100%;}</style>
</head><body>
...
<script>
async function main(){
  const res = await fetch("data/ctr.json"); const data = await res.json();
